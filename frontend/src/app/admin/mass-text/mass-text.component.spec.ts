...
describe("Component: MassText", () => {
  ...
  it("should calculate the correct message", async(() => {
    ...
    expect(component.preview).toEqual(
      "Hello, Mary - Reply 'NOFUN' to stop these messages"
    );
  }));
  ...
  it("should update the preview message when triggered", async(() => {
    ...
    component.ctrlTemplate.setValue(
      "Hello {{ name }}. How are you today, {{ name }}?"
    );
    component.updateTemplate();

    expect(component.preview).toEqual(
      "Hello Mary. How are you today, Mary? - Reply 'NOFUN' to stop these messages"
    );
  }));
  ...
  it("should calculate the correct number of characters remaining", async(() => {
    ...
    expect(component.numCharsUsed).toEqual(
      messageNoPlaceholderLength + nameLength + nameLength + optOutLength
    );
  }));
});
