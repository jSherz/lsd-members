...
describe("Component: CourseAddComponent", () => {
  ...
  it("should show a throbber while the request is running", async(
    inject([CourseAddComponent], (component: CourseAddComponent) => {
      ...
      StubCourseService.createDontCompleteSubject.next(
        new CourseCreateResponse(
          true,
          null,
          "2157f47e-6feb-476d-a6fd-29f68421406d"
        )
      );
  ...
  it("should show the correct error against number of spaces if it's invalid", async(
    inject([CourseAddComponent], (component: CourseAddComponent) => {
  ...
  it("should show any other API error (not invalid num spaces) against the general field / key", async(
    inject([CourseAddComponent], (component: CourseAddComponent) => {
      expect(component.errors).toBeUndefined();
  ...
  it("should translate the invalid number of spaces error correctly", async(
    inject([CourseAddComponent], (component: CourseAddComponent) => {
      expect(component.translate("error.invalidNumSpaces")).toEqual(
        "Invalid number of spaces - a course must have at " +
          "least one space and at most 50."
      );
    })
  ));

  it("should return the translation key if no translation is known", async(
    inject([CourseAddComponent], (component: CourseAddComponent) => {
      expect(component.translate("error.su138rojsdk8y13yrsdff")).toEqual(
        "error.su138rojsdk8y13yrsdff"
      );
    })
  ));
});
