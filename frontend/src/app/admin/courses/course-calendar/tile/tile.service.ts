...
export class TileService {
  ...
  getTiles(month: moment.Moment, rawCurrentDay: moment.Moment): Tile[] {
    ...
    const daysOfNextMonth =
      TILES_PER_CALENDAR - daysOfPrimaryMonth - daysOfPrevMonth;

    const nextMonth = firstDay.clone().add(1, "months");

    return this.rangeOfDates(
      firstDay.clone().subtract(daysOfPrevMonth, "days"),
      daysOfPrevMonth
    )
  ...
  private rangeOfDates(
    startingDate: moment.Moment,
    rangeSize: number
  ): moment.Moment[] {
    return Array(rangeSize)
      .fill(0)
      .map((_, offset) => startingDate.clone().add(offset, "days"));
  }
  ...
  private toTile(
    ...
    const isNextMonth =
      inputDate.isSame(nextMonth) || inputDate.isAfter(nextMonth);
